...
This project uses **LangGraph** because the assignment requires a real agentic workflow with state, branching behavior, retrieval, and tool execution. A normal chatbot loop can answer messages, but LangGraph makes the workflow explicit and easier to test. The agent is built with LangGraph's `StateGraph`, which is the stateful graph API inside LangGraph. The graph has three main nodes. First, `retrieve_knowledge` loads relevant AutoStream facts from the local JSON knowledge base. Second, `understand_user` classifies the user's intent and extracts lead details such as name, email, and creator platform. Third, `generate_response` answers from retrieved knowledge, asks for missing lead fields, or triggers the mock lead capture tool once all required details exist. GPT-4o-mini is used through `langchain-openai` when `OPENAI_API_KEY` is available, while a deterministic fallback keeps the demo runnable locally. State is managed through a typed dictionary that carries conversation history, detected intent, retrieved documents, collected lead fields, missing fields, and whether lead capture has already run. Because the same state object is passed back into the graph on every turn, the agent remembers context across 5-6 messages and avoids premature tool execution.
